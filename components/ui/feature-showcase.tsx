...
export function FeatureShowcase({ 
  ...
  const getStatusColor = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'enrolled': return 'text-purple-400 border-purple-400'
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="text-center space-y-2">
        <h2 className={`font-bold bg-gradient-to-r from-primary to-blue-400 bg-clip-text text-transparent ${compact ? 'text-2xl' : 'text-3xl'}`}>
          {title}
        </h2>
        <p className={`text-slate-400 ${compact ? 'text-sm' : 'text-lg'}`}>
          {subtitle}
        </p>
      </div>
      ...
      <div className={`grid gap-4 ${compact ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'}`}>
        {featuresToShow.map((feature, index) => (
          <Card key={index} className="bg-slate-800/50 border-slate-700 hover:border-primary/50 transition-all group cursor-pointer">
            <CardHeader className={compact ? 'p-4' : 'p-6'}>
              <div className="flex items-start justify-between mb-3">
                <div className={`${compact ? 'w-8 h-8' : 'w-10 h-10'} bg-slate-700/50 rounded-lg flex items-center justify-center`}>
                  <feature.icon className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} ${feature.color}`} />
                </div>
                <Badge variant="outline" className={`${getStatusColor(feature.status)} ${compact ? 'text-xs' : 'text-sm'}`}>
                  {feature.status}
                </Badge>
              </div>
