...
// Get current directory using ES modules syntax
...
async function createDeploymentPackage() {
  try {
    ...
    // Add client and server directories (filtered)
    console.log('📁 Adding client directory...');
    archive.directory('client', 'client', entry => shouldInclude(entry.name));
    
    console.log('📁 Adding server directory...');
    archive.directory('server', 'server', entry => shouldInclude(entry.name));
    
    console.log('📁 Adding shared directory...');
    archive.directory('shared', 'shared', entry => shouldInclude(entry.name));
...
function printDeploymentInstructions() {
  ...
  console.log('5. Start the application: npm run start:prod');
