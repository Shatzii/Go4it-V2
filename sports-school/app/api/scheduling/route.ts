...
async function generateBlockSchedule(schoolId: string | null, gradeLevel: string | null) {
  ...
  const schedule =
    scheduleTemplates[schoolId as keyof typeof scheduleTemplates] ||
    scheduleTemplates['primary-school'];
...
async function getAvailableCourses(schoolId: string | null, gradeLevel: string | null) {
  ...
  const courses =
    courseLibrary[schoolId as keyof typeof courseLibrary]?.[gradeLevel as keyof any] || [];
