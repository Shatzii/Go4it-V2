...
export async function GET(request: NextRequest) {
  try {
    ...
    // Add fallback for missing parameters
    if (!type) {
      return NextResponse.json(
        {
          error: 'Missing type parameter',
          availableTypes: ['dashboard', 'users', 'schools', 'compliance'],
        },
        { status: 400 },
      );
    }
    ...
    switch (type) {
      ...
      case 'users':
        try {
          ...
          return NextResponse.json({ error: 'Unable to fetch users', users: [] }, { status: 200 });
        }
  ...
  try {
    ...
    return {
      ...
      neurodiversityStats: {
        ...
        other: users.filter(
          (u) =>
            u.neurotype && !['ADHD', 'dyslexia', 'autism', 'neurotypical'].includes(u.neurotype),
        ).length,
      },
      enrollmentTypes: {
    ...
    texasEducationCode: {
      ...
      sections: {
        ...
        'TEC_28.0211': {
          compliant: true,
          score: 96,
          description: 'Dyslexia and Related Disorders',
        },
