...
function getSubjectDescription(subject: string): string {
  const descriptions = {
    english_language_arts:
      'Comprehensive language arts including reading, writing, speaking, and listening skills',
...
function getKeySkills(subject: string, grade: string): string[] {
  ...
  return (
    subjectSkills?.[grade as keyof typeof subjectSkills] || ['General skills for this subject']
  );
}
...
function getAssessmentMethods(subject: string, grade: string): string[] {
  ...
  let assessments = baseAssessments[subject as keyof typeof baseAssessments] || [
    'General assessments',
  ];
