...
async function generateUserAnalytics(
  ...
  const progress = await storage.getProgressByUser(parseInt(userId));
  const assessments = await storage.getSubmissionsByUser(parseInt(userId));
  const achievements = []; // Temporary fix for build
  const enrollments = [];

  // Calculate learning metrics
  const totalLessons = progress.length * 10; // Estimate based on progress records
  const completedLessons = progress.filter((p) => p.score && p.score > 0).length;
  ...
  const recentAssessments = assessments.filter(
    (a) => new Date(a.submittedAt || new Date()) >= startDate,
  );
  ...
  const currentStreak = Math.min(progress.length, 7); // Estimate based on recent activity
