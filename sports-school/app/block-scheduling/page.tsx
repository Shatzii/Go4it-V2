...
export default function DynamicBlockSchedulingPlanner() {
        ...
        spring2025: [
          {
            period: 1,
            courseId: 'theater2',
            courseName: 'Advanced Theater Performance',
            room: 'Theater',
            teacher: 'Ms. Barrett',
            credits: 1,
          },
          {
            period: 2,
            courseId: 'chardev',
            courseName: 'Character Development',
            room: 'A102',
            teacher: 'Ms. Barrett',
            credits: 0.5,
          },
          ...
          },
          {
            period: 4,
            courseId: 'career',
            courseName: 'Career Investigation',
            room: 'B101',
            teacher: 'Ms. Smith',
            credits: 1,
          },
      ...
      schedules: {
        fall2024: [
          ...
          },
          {
            period: 4,
            courseId: 'capstone',
            courseName: 'Theater Capstone Project',
            room: 'Theater',
            teacher: 'Ms. Barrett',
            credits: 1,
          },
  ...
  const validateSchedule = (studentId, semester) => {
    ...
    schedule.forEach((scheduledCourse) => {
      const course = Object.values(courseCatalog)
        .flat()
        .find((c) => c.id === scheduledCourse.courseId);
      if (course && course.prereq) {
        const missingPrereqs = course.prereq.filter(
    ...
    const requiredCourses = Object.values(courseCatalog)
      .flat()
      .filter((c) => c.required && c.grade.includes(student.grade));
    requiredCourses.forEach((req) => {
      if (
        !student.completedCourses.includes(req.id) &&
        !schedule.some((s) => s.courseId === req.id)
      ) {
    ...
    return Object.values(courseCatalog)
      ...
      .filter((course) => {
        ...
        if (course.prereq && course.prereq.length > 0) {
          const hasPrereqs = course.prereq.every((prereq) =>
  ...
  const removeCourseFromSchedule = (studentId, semester, period) => {
    setStudentDatabase((prev) => ({
      ...
      [studentId]: {
        ...
        schedules: {
          ...prev[studentId].schedules,
          [semester]: (prev[studentId].schedules[semester] || []).filter(
            (s) => s.period !== period,
          ),
        },
      },
  ...
  const getStudentList = () => {
    return Object.values(studentDatabase).filter(
      (student) => selectedGrade === 'all' || student.grade.toString() === selectedGrade,
    );
  };
  ...
  const getCourseColor = (course) => {
    ...
    const courseData = Object.values(courseCatalog)
      .flat()
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-blue-900 to-purple-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-white mb-2">Dynamic Block Scheduling Planner</h1>
          <p className="text-blue-200 text-lg">
            4×4 Block Schedule Management | Stage Prep Academic Planning
          </p>
          <div className="mt-4 flex justify-center gap-4 flex-wrap">
            <Badge className="bg-blue-600/20 text-blue-300 px-4 py-2">4 Blocks per Semester</Badge>
            <Badge className="bg-purple-600/20 text-purple-300 px-4 py-2">Year-Long Courses</Badge>
            <Badge className="bg-green-600/20 text-green-300 px-4 py-2">Texas Compliant</Badge>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {getStudentList().map((student) => {
                  ...
                  const totalCredits = schedule.reduce(
                    (sum, course) => sum + (course.credits || 1),
                    0,
                  );
                        ...
                        <div className="space-y-3">
                          {timeSlots.map((slot) => {
                            ...
                            return (
                              <div
                                key={slot.period}
                                className={`p-3 rounded-lg border ${getCourseColor(course)} border-white/10`}
                              >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2">
                  <Card className="bg-white/10 border-white/20 text-white">
                    <CardHeader>
                      <CardTitle>
                        {studentDatabase[selectedStudent]?.name} -{' '}
                        {selectedSemester === 'fall2024' ? 'Fall 2024' : 'Spring 2025'}
                      </CardTitle>
                      <CardDescription className="text-blue-200">
                      ...
                      <div className="space-y-4">
                        {timeSlots.map((slot) => {
                          ...
                          return (
                            <div
                              key={slot.period}
                              className={`p-4 rounded-lg border ${getCourseColor(course)} border-white/10`}
                            >
                              <div className="flex items-center justify-between mb-3">
                                ...
                                {editingSchedule && (
                                  <div className="flex gap-2">
                                    ...
                                    <Dialog
                                      open={showCourseDialog && selectedPeriod === slot.period}
                                      onOpenChange={(open) => {
                    ...
                    <CardContent className="space-y-3 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-purple-300">
                          {studentDatabase[selectedStudent]?.pathway}
                        </span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="space-y-4 max-h-96 overflow-y-auto">
                        {Object.entries(courseCatalog).map(([category, courses]) => {
                          const availableCourses = courses.filter((course) => {
                            ...
                            return (
                              course.grade.includes(student.grade) &&
                              !student.completedCourses.includes(course.id) &&
                              !(student.schedules[selectedSemester] || []).some(
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            ...
                            <p className="text-blue-200 text-sm">
                              Grade {student.grade} • {student.pathway}
                            </p>
                          </div>
                        ...
                        {validation.warnings.length > 0 && (
                          <div className="mb-4 p-3 bg-yellow-600/20 rounded-lg border border-yellow-600/30">
                            ...
                            {validation.warnings.map((warning, idx) => (
                              <div key={idx} className="text-sm text-yellow-200 mb-1">
                                • {warning}
                              </div>
                            ))}
                          </div>
                        ...
                        {validation.suggestions.length > 0 && (
                          <div className="mb-4 p-3 bg-blue-600/20 rounded-lg border border-blue-600/30">
                            ...
                            {validation.suggestions.slice(0, 3).map((suggestion, idx) => (
                              <div key={idx} className="text-sm text-blue-200 mb-1">
                                • {suggestion}
                              </div>
                            ))}
                            {validation.suggestions.length > 3 && (
                              <div className="text-sm text-blue-200">
                                ... and {validation.suggestions.length - 3} more
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        {!hasIssues && validation.suggestions.length === 0 && (
                          <div className="p-3 bg-green-600/20 rounded-lg border border-green-600/30">
                            <div className="text-green-300 text-sm">
                              ...
                              <br />✓ Graduation requirements on track
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="text-2xl font-bold">
                    {(
                      getStudentList().reduce((sum, student) => {
                        ...
                        return (
                          sum +
                          schedule.reduce((credits, course) => credits + (course.credits || 1), 0)
                        );
                      }, 0) / Math.max(getStudentList().length, 1)
                    ).toFixed(1)}
                  </div>
              ...
              <Card className="bg-white/10 border-white/20 text-white">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-green-300">
                    {Math.round(
                      (getStudentList().filter((student) => {
                        const schedule = student.schedules[selectedSemester] || [];
                        return schedule.length === 4;
                      }).length /
                        Math.max(getStudentList().length, 1)) *
                        100,
                    )}
                    %
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="p-4 bg-purple-600/10 rounded-lg border border-purple-600/30">
                    ...
                    <p className="text-sm text-purple-200 mb-4">
                      Acting, voice, and stage performance focus
                    </p>
                  ...
                  <div className="p-4 bg-green-600/10 rounded-lg border border-green-600/30">
                    ...
                    <p className="text-sm text-green-200 mb-4">
                      Playwriting and theatrical storytelling
                    </p>
