...
class LearningStreakSystem {
  ...
  createUIElements() {
    ...
    badgeContainer.innerHTML =
      '<div class="badge-title">Your Badges</div><div class="badge-grid"></div>';

    // Create achievement container
    const achievementContainer = document.createElement('div');
    achievementContainer.className = 'achievement-container';
    achievementContainer.innerHTML =
      '<div class="achievement-title">Recent Achievements</div><div class="achievement-list"></div>';
    ...
    const targetContainer =
      document.querySelector('.dashboard-stats') || document.querySelector('main') || document.body;
  ...
  calculateLevelProgress() {
    ...
    const progress =
      ((points - previousLevelPoints) / (nextLevelPoints - previousLevelPoints)) * 100;
  ...
  checkForAchievements(type, data) {
    ...
    Object.entries(this.achievements).forEach(([id, achievement]) => {
      ...
      switch (achievement.type) {
        ...
        case 'activity':
          if (
            this.activityLog.filter((a) => a.type === achievement.activityType).length >=
        ...
        case 'challenge':
          if (type === 'challenge' && data && data.difficulty === achievement.difficulty) {
            const challengeCount = this.activityLog.filter(
              (a) =>
