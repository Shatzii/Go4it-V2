    ...
    return (
      <div className="bg-yellow-900/20 border border-yellow-500 rounded-lg p-6 text-center">
        ...
        <p className="text-gray-300 mb-4">
          Upgrade to VR Premium or AI Enhanced to earn blockchain-verified certificates
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      {!walletConnected && (
        <div className="bg-blue-900/20 border border-blue-500 rounded-lg p-6 text-center">
          ...
          <p className="text-gray-300 mb-4">
            Connect a Web3 wallet to mint and manage your NFT achievements
          </p>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {earnedAchievements.map((achievement) => (
          <div
            key={achievement.id}
            className="bg-gradient-to-br from-green-900/50 to-emerald-900/50 border border-green-500 rounded-lg p-6"
          >
        ...
        {availableAchievements
          ...
          .map((achievement) => (
            <div
              key={achievement.id}
              className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-600 rounded-lg p-6"
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-gray-400 font-bold">{achievement.title}</h3>
                <span
                  className={`px-2 py-1 rounded text-xs ${getLevelColor(achievement.level)} opacity-60`}
                >
                  {achievement.level.toUpperCase()}
                </span>
              </div>
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-green-400">
            {Math.round(blockchainMetrics.verification_score * 100)}%
          </span>
        </div>
      </div>
    </div>
