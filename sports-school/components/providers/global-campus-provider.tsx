...
export function GlobalCampusProvider({ children }: { children: ReactNode }) {
  ...
  const scheduleGlobalSession = async (sessionData: Partial<GlobalSession>): Promise<string> => {
    try {
      const response = await fetch('/api/global/sessions/schedule', {
        ...
        body: JSON.stringify({
          ...sessionData,
          optimal_time: timeZoneManager.getOptimalMeetingTime(
            sessionData.participating_campuses || [],
          ),
        }),
      });
  ...
  return <GlobalCampusContext.Provider value={value}>{children}</GlobalCampusContext.Provider>;
}
...
export function GlobalCampusDashboard() {
  const { campuses, currentCampus, globalSessions, switchCampus, timeZoneManager } =
    useGlobalCampus();
    ...
    return (
      <div className="bg-blue-900/20 border border-blue-500 rounded-lg p-6 text-center">
        ...
        <p className="text-gray-300">
          Upgrade to access multi-campus features and international programs
        </p>
      </div>
    );
  }
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-green-400">
                  {campus.enrollment.current_students}/{campus.capacity.students}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-purple-400">
                  {campus.enrollment.housing_occupied}/{campus.capacity.housing}
                </span>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="text-pink-400">
                    {session.students_enrolled}/{session.max_capacity}
                  </span>
                </div>
              </div>
