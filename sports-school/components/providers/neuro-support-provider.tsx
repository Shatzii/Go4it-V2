...
export function NeuroSupportProvider({ children }: { children: ReactNode }) {
  ...
  return <NeuroSupportContext.Provider value={value}>{children}</NeuroSupportContext.Provider>;
}
...
export function NeuroSupportDashboard() {
  const { profile, currentFocusSession, requestSensoryBreak, startFocusSession, emergencySupport } =
    useNeuroSupport();
    ...
    return (
      <div className="bg-purple-900/20 border border-purple-500 rounded-lg p-6 text-center">
        ...
        <p className="text-gray-300">
          Upgrade to access specialized learning accommodations and AI coaching
        </p>
      </div>
    );
  }
              ...
              <div className="flex items-center">
                <div className="bg-gray-700 rounded-full h-2 w-20 mr-2">
                  <div
                    ...
                    style={{
                      width: `${profile.support_needs.cognitive.executive_function_level * 10}%`,
                    }}
                  />
                </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-green-400 font-semibold">
                Active Session: {currentFocusSession.subject}
              </span>
              <span className="text-yellow-400">
                {Math.round(
                  (Date.now() - new Date(currentFocusSession.start_time).getTime()) / 60000,
                )}{' '}
                min
              </span>
            </div>
