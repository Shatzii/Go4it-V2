...
function logSecuritySummary(): void {
  ...
  console.log(
    `Database: ${hasDbUrl ? '✅ Configured' : '❌ Missing'} ${useMemory ? '(Memory Storage)' : '(PostgreSQL)'}`,
  );
  ...
  console.log(
    `AI Services: Anthropic ${hasAnthropic ? '✅' : '❌'}, OpenAI ${hasOpenAI ? '✅' : '❌'}`,
  );
  ...
  console.log(
    `External APIs: SMTP ${hasSmtp ? '✅' : '❌'}, AWS ${hasAws ? '✅' : '❌'}, Stripe ${hasStripe ? '✅' : '❌'}`,
  );
