...
function hasOverallScore(score: any): score is AssessmentScore {
  return score && typeof score === 'object' && typeof score.overall === 'number';
}
...
export async function createLearningProfile(
  ...
  const enhancedProfile: InsertLearningProfile = {
    ...
    contentPreferences: profile.contentPreferences ||
      DEFAULT_PREFERENCES[profile.neurodivergentType]?.contentPreferences || {
        preferredFormat: 'standard',
        contentDensity: 'moderate',
        breakFrequency: 'moderate',
      },
...
export async function generateProfileFromAssessments(userId: number): Promise<LearningProfile> {
  ...
  const learningStyleAssessment = assessments.find(
    (a) => a.assessmentType === AssessmentType.LEARNING_STYLE,
  );
...
export async function saveAssessmentResult(
  assessment: InsertAssessmentResult,
...
function determineNeurodivergentType(assessments: AssessmentResult[]): NeurodivergentType {
  ...
  const readingScore =
    readingSkills && hasOverallScore(readingSkills.score) ? readingSkills.score.overall : 0;

  // Check if we have attention assessment
  const attentionAssessment = assessments.find(
    (a) => a.assessmentType === AssessmentType.ATTENTION_ASSESSMENT,
  );
  const attentionScore =
    attentionAssessment && hasOverallScore(attentionAssessment.score)
      ? attentionAssessment.score.overall
      : 0;

  // Check if we have social communication assessment
  const socialCommunication = assessments.find(
    (a) => a.assessmentType === AssessmentType.SOCIAL_COMMUNICATION,
  );
  const socialScore =
    socialCommunication && hasOverallScore(socialCommunication.score)
      ? socialCommunication.score.overall
      : 0;
  ...
  if (readingScore > 0.7 && readingScore >= attentionScore && readingScore >= socialScore) {
    ...
    attentionScore > 0.7 &&
    attentionScore >= readingScore &&
    attentionScore >= socialScore
  ) {
...
function generateContentPreferences(
  ...
  const readingAssessment = assessments.find(
    (a) => a.assessmentType === AssessmentType.READING_SKILLS,
  );
  const attentionAssessment = assessments.find(
    (a) => a.assessmentType === AssessmentType.ATTENTION_ASSESSMENT,
  );
