...
export class SelfHostedAnthropicEngine {
  ...
  private async ensureModelAvailability() {
    try {
      ...
      const modelsConfig = {
        ...
        fallbackModels: ['microsoft/DialoGPT-medium', 'distilbert-base-uncased', 'gpt2-medium'],
      };
  ...
  private selectBestModel(
    ...
    if (
      userMessage.includes('law') ||
      userMessage.includes('legal') ||
      userMessage.includes('bar exam')
    ) {
    ...
    if (
      userMessage.includes('adhd') ||
      userMessage.includes('autism') ||
      userMessage.includes('dyslexia') ||
      userMessage.includes('neurodivergent')
    ) {
      return this.models.get('claude-neurodivergent-specialist')!;
    }

    if (
      userMessage.includes('elementary') ||
      userMessage.includes('superhero') ||
      userMessage.includes('grade 1') ||
      userMessage.includes('grade 6')
    ) {
  ...
  private getEducationalTemplates(model: EducationalModel): any {
    const templates = {
      'claude-educational-primary': {
        lesson:
          "Hey there, Super Student! 🦸‍♂️ Let me help you learn about {topic}. We'll make it fun and easy to understand!",
      ...
      'claude-educational-secondary': {
        lesson:
          "Welcome to today's exploration of {topic}. Let's dive deep into the fascinating world of {subject}.",
        explanation:
          'To understand {concept}, we need to examine its key components and relationships:',
        practice: 'Your challenge: Apply this knowledge to {scenario}',
        encouragement:
          "Excellent critical thinking! You're developing sophisticated analytical skills.",
      },
      'claude-legal-education': {
        lesson:
          "Today we'll examine the legal principles surrounding {topic}. Understanding this concept is crucial for {application}.",
      ...
      'claude-language-tutor': {
        ...
        explanation:
          'In {language}, {concept} is expressed as: {translation}. The cultural context shows us:',
      ...
      'claude-neurodivergent-specialist': {
        lesson:
          "Let's learn about {topic} in a way that works best for your brain! We'll take it step by step.",
        ...
        encouragement:
          "You're doing wonderfully! Every brain learns differently, and yours is amazing.",
      },
    };
  ...
  private generateAssessmentContent(
    model: EducationalModel,
    topic: string,
    templates: any,
  ): string {
  ...
  private generateDefaultContent(model: EducationalModel, topic: string, templates: any): string {
    return (
      `I'd be happy to help you with ${topic}! What specific aspect would you like to explore?\n\n` +
  ...
  private generateTopicExplanation(topic: string, model: EducationalModel): string {
    ...
    if (model.id === 'claude-educational-primary') {
      return (
        `${topic} is like... (imagine a fun comparison here!)\n` +
    ...
    return (
      `${topic} involves several key principles:\n` +
  ...
  private generatePracticeActivities(topic: string, model: EducationalModel): string {
    if (model.id === 'claude-educational-primary') {
      return (
        `1. 🎯 Super Challenge: Simple activity with ${topic}\n` +
    ...
    return (
      `1. Analysis Exercise: Examine ${topic} in depth\n` +
  ...
  private generateLearningStrategies(topic: string, model: EducationalModel): string {
    const strategies = model.capabilities
      .map((capability) => `• ${capability.charAt(0).toUpperCase() + capability.slice(1)} approach`)
      .join('\n');
  ...
  private generateCreativeProjects(topic: string, model: EducationalModel): string {
    if (model.id === 'claude-educational-primary') {
      return (
        `1. 🎨 Draw a superhero who uses ${topic}\n` +
