  ...
  public async generateContent(request: ContentRequest): Promise<ContentResponse> {
    try {
      ...
      const contentResponse: ContentResponse = {
        ...
        timestamp: new Date(),
        adaptations: request.learningProfile
          ? this.getAdaptationsApplied(request.learningProfile)
          : undefined,
        metadata: {
...
- If the student writes in English but should be practicing ${this.teachingLanguage}, gently encourage them to try in ${this.teachingLanguage}`;
  ...
  private determineTeachingLanguage(specialization: string): TeachingLanguage {
    ...
    if (specializationLower.includes('spanish')) {
      ...
      specializationLower.includes('chinese') ||
      specializationLower.includes('mandarin')
    ) {
  ...
  private determineLessonType(topic: string): LessonType {
    ...
    if (
      ...
      topicLower.includes('dialogue')
    ) {
      return LessonType.CONVERSATION;
    } else if (
      topicLower.includes('reading') ||
      topicLower.includes('text') ||
      topicLower.includes('passage')
    ) {
      return LessonType.READING;
    } else if (
      topicLower.includes('writing') ||
      topicLower.includes('essay') ||
      topicLower.includes('composition')
    ) {
      return LessonType.WRITING;
    } else if (
      topicLower.includes('pronunciation') ||
      topicLower.includes('accent') ||
      topicLower.includes('phonetics')
    ) {
      return LessonType.PRONUNCIATION;
    } else if (
      topicLower.includes('culture') ||
      topicLower.includes('tradition') ||
      topicLower.includes('custom')
    ) {
      return LessonType.CULTURE;
    } else if (
      topicLower.includes('test') ||
      topicLower.includes('exam') ||
  ...
  public static createDefaultProfile(language: TeachingLanguage): ProfessorProfile {
    ...
    switch (language) {
      case TeachingLanguage.SPANISH:
        ...
        description =
          'A native Spanish speaker from Madrid with over 15 years of teaching experience. Profesora Martínez specializes in Spanish language acquisition for English speakers, with expertise in Spanish grammar, conversation, and Hispanic cultures.';
        systemPrompt =
      ...
      case TeachingLanguage.GERMAN:
        ...
        description =
          'A native German speaker from Berlin with extensive experience teaching German as a foreign language. Professor Weber specializes in German grammar, pronunciation, and modern German culture and literature.';
        systemPrompt =
      ...
      case TeachingLanguage.CHINESE:
        ...
        description =
          'A native Mandarin speaker from Beijing with specialized training in teaching Chinese as a foreign language. Professor Li is an expert in Chinese characters, tones, and cultural nuances essential for effective communication.';
        systemPrompt =
      ...
      default:
        ...
        description =
          'A TESOL-certified English language educator with over a decade of experience teaching English to speakers of other languages. Dr. Johnson specializes in communicative language teaching, English grammar, and cross-cultural communication.';
        systemPrompt =
