...
export function applyImprovedLanguageSchoolMethods() {
  ...
  anyStorage.updateUserVocabularyProgress = async (
    userId: number,
    itemId: number,
    progressData: any,
  ) => {
    try {
      ...
      if (existingProgress) {
        ...
        console.log(
          `✅ Updated vocabulary progress in database for user ${userId} and item ${itemId}`,
        );
        ...
        console.log(
          `✅ Created vocabulary progress in database for user ${userId} and item ${itemId}`,
        );
        return newProgress;
      }
    } catch (error) {
      console.error(
        `❌ Database error updating vocabulary progress for user ${userId} and item ${itemId}:`,
        error,
      );

      // Fall back to memory storage
      console.log(
        `ℹ️ Falling back to memory for vocabulary progress for user ${userId} and item ${itemId}`,
      );
      ...
      if (existingProgress) {
        ...
        anyStorage.userVocabularyProgressCurrentId =
          (anyStorage.userVocabularyProgressCurrentId || 1) + 1;
