      ...
      if (res.get('Content-Type')?.includes('application/json')) {
        ...
        try {
          if (typeof body === 'string') {
            ...
            console.log(
              `[${timestamp}] Response Body: ${JSON.stringify(parsed, null, 2).substring(0, 500)}...`,
            );
          } else if (typeof body === 'object') {
            console.log(
              `[${timestamp}] Response Body: ${JSON.stringify(body, null, 2).substring(0, 500)}...`,
            );
          }
        } catch (error) {
          // Non-JSON response
          console.log(
            `[${timestamp}] Response is not valid JSON or cannot be logged (length: ${body?.length || 0})`,
          );
        }
      } else {
        // Non-JSON response
        console.log(
          `[${timestamp}] Non-JSON Response: Content-Type ${res.get('Content-Type')}, Status ${res.statusCode}`,
        );
      }
    }
