...
export function applyCMSMethods(storage: IStorage): IStorage {
  ...
  function initializeMemoryData() {
    ...
    if (memData.schools.length === 0) {
      ...
      const ndElementarySchool: School = {
        ...
        description:
          'A superhero-themed elementary school for neurodivergent learners from preschool to 6th grade.',
        type: 'elementary',
      ...
      const ndHighSchool: School = {
        ...
        description:
          'A cutting-edge middle and high school designed specifically for neurodivergent students.',
        type: 'high',
        gradeRange: '7th-12th',
      ...
      const lawSchool: School = {
        ...
        slug: 'shatzii-law',
        description:
          'A comprehensive online law school offering UAE bar preparation and specialized legal education.',
        type: 'law',
        gradeRange: 'Graduate',
      ...
      const languageSchool: School = {
        ...
        description:
          'An immersive language learning school offering courses in multiple languages with AI conversation practice.',
        type: 'language',
        gradeRange: 'All Ages',
      ...
      const homePage: Page = {
        ...
        slug: 'home',
        content:
          '<h1>Welcome to the Future of Education</h1><p>ShatziiOS is a revolutionary educational platform designed to provide personalized learning experiences for all students, with special focus on neurodivergent learners.</p>',
        schoolId: null,
        type: 'home',
        isPublished: true,
      ...
      const elementaryTeacher: AiTeacher = {
        ...
        subject: 'Science',
        description:
          'A bright and enthusiastic science teacher who specializes in making complex concepts simple and fun.',
        avatarUrl: '/assets/avatars/ms-stellar.png',
        teachingStyle:
          'Visual, hands-on, and experiment-based learning with lots of positive reinforcement.',
        specialties: 'Astronomy, Biology, Environmental Science',
        neurotypeTailoring:
          'Provides extra visual aids for visual learners, breaks down concepts into manageable chunks for ADHD students, and uses special interest-based examples for autistic students.',
        ...
        systemPrompt:
          'You are Ms. Stellar, an enthusiastic elementary school science teacher who makes learning fun and accessible for all students.',
        personalityPrompt:
          'You are bright, encouraging, and excited about science. You meet students at their level and celebrate small victories. You explain concepts in simple terms with lots of visual examples.',
        active: true,
        createdAt: new Date(),
        updatedAt: new Date(),
      };
      ...
      const lawTeacher: AiTeacher = {
        ...
        description:
          'A distinguished professor of commercial law with extensive practical experience in UAE commercial regulations.',
        avatarUrl: '/assets/avatars/prof-chambers.png',
        teachingStyle:
          'Case-based learning with Socratic method and practical application exercises.',
        specialties: 'UAE Commercial Code, Contract Law, Corporate Governance',
        neurotypeTailoring:
          'Provides structured outlines for executive function support, records sessions for review, and offers flexible assessment options.',
        gradeLevel: 'Graduate',
        schoolId: 3,
        aiModel: 'claude-3-7-sonnet-20250219', // the newest Anthropic model is "claude-3-7-sonnet-20250219" which was released February 24, 2025
        systemPrompt:
          'You are Professor Chambers, an expert in UAE Commercial Law with decades of experience in both academia and legal practice.',
        personalityPrompt:
          'You are precise, thoughtful, and methodical. You challenge students with complex scenarios but guide them through the reasoning process. You frequently reference practical applications of legal principles.',
        active: true,
        createdAt: new Date(),
        updatedAt: new Date(),
      };
      ...
      const elementaryResource: Resource = {
        ...
        description:
          'A collection of fun, superhero-themed science experiments that can be done at home with simple materials.',
        type: 'document',
      ...
      const lawResource: Resource = {
        ...
        description:
          'A comprehensive study guide for the commercial code section of the UAE Bar Exam.',
        type: 'document',
  ...
  storage.updateNeurodivergentSchool = async (
    id: number,
    ...
    if (memData) {
      ...
      const school = memData.schools.find(
        (s) => s.id === memData.neurodivergentSchools[index].schoolId,
      );
      return { ...memData.neurodivergentSchools[index], school };
    } else {
      const result = await db
        .update(neurodivergentSchools)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(neurodivergentSchools.id, id))
        .returning();
    ...
    if (memData) {
      ...
      if (filters?.schoolId) {
        filteredTeachers = filteredTeachers.filter(
          (teacher) => teacher.schoolId === filters.schoolId,
        );
      }
  ...
  // Resources
  storage.getResources = async (filters?: {
    schoolId?: number;
    type?: string;
    schoolType?: string;
  }) => {
    if (memData) {
      ...
      if (filters?.schoolId) {
        filteredResources = filteredResources.filter(
          (resource) => resource.schoolId === filters.schoolId,
        );
      }
      ...
      if (filters?.schoolType) {
        filteredResources = filteredResources.filter(
          (resource) => resource.schoolType === filters.schoolType,
        );
      }
