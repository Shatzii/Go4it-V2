...
export interface ILearningMiniGameStorage {
  ...
  updateMiniGame(
    id: number,
    game: Partial<InsertLearningMiniGame>,
  ): Promise<LearningMiniGame | undefined>;
  ...
  updateMiniGameContent(
    id: number,
    content: Partial<InsertMiniGameContent>,
  ): Promise<MiniGameContent | undefined>;
  deleteMiniGameContent(id: number): Promise<boolean>;

  // User progress operations
  getUserMiniGameProgress(userId: number): Promise<UserMiniGameProgress[]>;
  getUserMiniGameProgressByGame(
    userId: number,
    gameId: number,
  ): Promise<UserMiniGameProgress | undefined>;
  createUserMiniGameProgress(progress: InsertUserMiniGameProgress): Promise<UserMiniGameProgress>;
  updateUserMiniGameProgress(
    id: number,
    progress: Partial<InsertUserMiniGameProgress>,
  ): Promise<UserMiniGameProgress | undefined>;
  completeMiniGame(
    userId: number,
    gameId: number,
  ...
  getTopPerformingMiniGames(
    limit?: number,
  ): Promise<{ gameId: number; title: string; averageScore: number; timesPlayed: number }[]>;
...
export class LearningMiniGameMemStorage implements ILearningMiniGameStorage {
  ...
  private initializeSampleData() {
    ...
    const sampleGames: InsertLearningMiniGame[] = [
      {
        ...
        timeLimit: 120,
        instructions:
          'Flip through the flashcards and answer as many as you can within the time limit!',
        rewardXp: 15,
        active: true,
        metadata: {
      ...
      },
      {
        ...
        timeLimit: 180,
        instructions:
          'Drag each term to its matching definition. Complete all matches before time runs out!',
        rewardXp: 20,
        active: true,
        metadata: {
          pairs: 10,
      ...
      },
      {
        ...
        timeLimit: 240,
        instructions:
          'Select the correct answer for each grammar question. Get as many right as possible!',
        rewardXp: 25,
        active: true,
        metadata: {
          questionCount: 10,
      ...
      },
      {
        ...
        timeLimit: 180,
        instructions:
          'Drag and drop the events to arrange them in the correct chronological order.',
        rewardXp: 30,
        active: true,
        metadata: {
          era: 'World War II',
    ...
    const flashcardGame = Array.from(this.miniGames.values()).find(
      (g) => g.gameType === 'flashcard',
    );
    if (flashcardGame) {
      const operations = ['+', '-', '×', '÷'];
      ...
      const grammarQuestions = [
        {
          ...
          correctIndex: 1,
          explanation:
            "The second option correctly uses commas to separate items in a list, including the Oxford comma before 'and'.",
        },
        {
          ...
          correctIndex: 1,
          explanation:
            "Adverbs modify verbs, adjectives, or other adverbs. 'Quickly' modifies how she ran.",
        },
        {
  ...
  async getUserMiniGameProgress(userId: number): Promise<UserMiniGameProgress[]> {
    return Array.from(this.userMiniGameProgress.values()).filter(
      (progress) => progress.userId === userId,
    );
  }

  async getUserMiniGameProgressByGame(
    userId: number,
    gameId: number,
  ): Promise<UserMiniGameProgress | undefined> {
    const userProgress = Array.from(this.userMiniGameProgress.values()).filter(
    ...
    return userProgress.sort(
      (a, b) => new Date(b.lastPlayed).getTime() - new Date(a.lastPlayed).getTime(),
    )[0];
  }

  async createUserMiniGameProgress(
    progress: InsertUserMiniGameProgress,
  ): Promise<UserMiniGameProgress> {
  ...
  async updateUserMiniGameProgress(
    id: number,
    progress: Partial<InsertUserMiniGameProgress>,
  ): Promise<UserMiniGameProgress | undefined> {
  ...
  // Activity tracking
  async recordUserMiniGameActivity(
    activity: InsertUserMiniGameActivity,
  ): Promise<UserMiniGameActivity> {
  ...
  async getTopPerformingMiniGames(
    limit: number = 5,
  ): Promise<{ gameId: number; title: string; averageScore: number; timesPlayed: number }[]> {
    ...
    return results.sort((a, b) => b.averageScore - a.averageScore).slice(0, limit);
  }
