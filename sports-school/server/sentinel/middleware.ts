...
const apiRateLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    ...
    sendAlert(AlertSeverity.MEDIUM, AlertType.RATE_LIMIT, `Rate limit exceeded for IP: ${ip}`, {
      path: req.path,
      method: req.method,
      headers: req.headers,
...
export function applySentinelSecurity(app: any): void {
  ...
  app.use((err: Error, req: Request, res: Response, next: NextFunction) => {
    ...
    import('./audit-log')
      .then(({ logErrorEvent, getRequestIP }) => {
        const user = (req as any).user?.username || 'anonymous';
        const ip = getRequestIP(req);
        const userAgent = req.headers['user-agent'] as string;

        logErrorEvent(`Error in ${req.method} ${req.path}`, err, user, ip, userAgent);
      })
      .catch((importError) => {
