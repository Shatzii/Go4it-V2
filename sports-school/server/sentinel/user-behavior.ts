...
export function recordUserActivity(
  ...
  // IP address
  profile.commonIPs.set(activity.ip, (profile.commonIPs.get(activity.ip) || 0) + 1);
  ...
  profile.commonAccessTimes.set(hourOfDay, (profile.commonAccessTimes.get(hourOfDay) || 0) + 1);

  // Day of week (0-6)
  const dayOfWeek = new Date(now).getDay();
  profile.commonDaysOfWeek.set(dayOfWeek, (profile.commonDaysOfWeek.get(dayOfWeek) || 0) + 1);
  ...
  profile.activityHistory.forEach((activity) => {
    ...
    if (existingSession) {
      ...
      const latestActivity = profile.activityHistory
        .filter((a) => a.sessionId === activity.sessionId)
        .reduce((latest, current) => (current.timestamp > latest.timestamp ? current : latest));
...
function detectAnomalies(
  ...
  if (profile.sessionId && profile.typicalRequestsPerSession > 0) {
    const sessionRequests = profile.activityHistory.filter(
...
function handleAnomalies(
  ...
  const highestSeverity = anomalies.reduce((highest, current) => {
    ...
    return severityValues[current.severity] > severityValues[highest] ? current.severity : highest;
  }, AlertSeverity.LOW);
...
function cleanupOldData(): void {
  ...
  for (const [username, profile] of userProfiles.entries()) {
    ...
    profile.anomalies = profile.anomalies.filter((anomaly) => anomaly.timestamp >= cutoffTime);
