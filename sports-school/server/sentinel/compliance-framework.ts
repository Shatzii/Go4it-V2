...
function initializeSecurityControls(): void {
  ...
  addSecurityControl({
    ...
    description:
      'Establish account creation, modification, enabling, disabling, and removal procedures',
    category: ControlCategory.ACCESS_CONTROL,
    status: ControlStatus.IMPLEMENTED,
  });
 ...
 */
export function getAllSecurityControls(filter?: {
  category?: ControlCategory;
  status?: ControlStatus;
}): SecurityControl[] {
...
export function getComplianceStatus(standard: ComplianceStandard): {
  ...
  for (const mapping of mappings) {
    ...
    const compliancePercentage =
      totalCount === 0 ? 0 : ((implementedCount + partialCount * 0.5) / totalCount) * 100;
  ...
  const overallCompliance = requirementCount === 0 ? 0 : totalPercentage / requirementCount;
...
export function generateComplianceReport(
  ...
  const requirementSummary: Record<
    string,
    {
      requirement: string;
      status: ControlStatus;
 ...
 */
export function getLatestComplianceReport(
  standard: ComplianceStandard,
