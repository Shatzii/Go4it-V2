...
router.get('/metrics', (req, res) => {
  try {
    ...
    const attackSources = [
      ...
      { country: 'Russia', count: 63, latitude: 61.524, longitude: 105.3188 },
      { country: 'Brazil', count: 42, latitude: -14.235, longitude: -51.9253 },
    ...
    if (scanResults.vulnerabilities && scanResults.vulnerabilities.length > 0) {
      ...
      highPriorityIssues.forEach((vulnerability: any) => {
        ...
        storeAlert({
          ...
          severity:
            vulnerability.severity === 'critical' ? AlertSeverity.CRITICAL : AlertSeverity.HIGH,
          type: alertType,
...
router.get('/generate-test-data', async (req, res) => {
  try {
    ...
    const fileAlert = {
      ...
      details: {
        fileName: 'assignment.js.exe',
        fileSize: '250KB',
        rejectionReason: 'executable file type',
      },
      timestamp: new Date().toISOString(),
      user: 'student1',
      ip: '8.8.8.8',
    ...
    const honeypotAlert = {
      ...
      details: {
        endpoint: '/api/v1/admin/system-debug',
        method: 'GET',
    ...
    const systemAlert = {
      ...
      details: {
        updatedBy: 'ceoadmin',
        changes: { rateLimitRequests: '100 -> 200', sessionTimeout: '60 -> 30' },
      },
      timestamp: new Date().toISOString(),
      user: 'ceoadmin',
      ip: '10.0.0.1',
    ...
    res.json({
      success: true,
      message: 'Test security data generated successfully',
      alertCount: 7,
    });
  } catch (error) {
...
router.post('/remediate-all', (req, res) => {
  ...
  try {
    ...
    const activeAlerts = recentAlerts.filter(
      (alert) => alert.status === 'active' || alert.status === undefined,
    );
    ...
    activeAlerts.forEach((alert) => {
      try {
        ...
        const moduleId =
          alert.details?.moduleId ||
          (alert.type === AlertType.AUTHENTICATION
            ? 'auth-security'
            : alert.type === AlertType.FILE_UPLOAD
              ? 'file-guard'
              : alert.type === AlertType.SQL_INJECTION
                ? 'db-monitor'
                : alert.type === AlertType.XSS
                  ? 'content-security'
                  : alert.type === AlertType.RATE_LIMIT
                    ? 'rate-limiter'
                    : alert.type === AlertType.HONEYPOT
                      ? 'honeypot'
                      : alert.type === AlertType.SUSPICIOUS_ACTIVITY
                        ? 'ip-blocker'
                        : 'security-scoring');
