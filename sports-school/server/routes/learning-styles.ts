...
router.post('/generate-persona', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      persona =
        sampleLearningPersonas[learningStyle as LearningStyleType] || sampleLearningPersonas.visual; // Default to visual if style not found

      // Add personalized interest strategies if userData has interests
      if (userData?.interests && userData.interests.length > 0) {
        persona.interestSpecificStrategies = userData.interests.map(
          (interest) => `Apply ${learningStyle} techniques when learning about ${interest}`,
        );
      }
    }
...
function calculateLearningStyle(responses: QuizResponse[]): string {
  ...
  const dominantStyles = Object.keys(styleCounts).filter(
    (style) => styleCounts[style] === maxCount,
  );
  ...
  try {
    ...
    try {
      ...
      const missingFields = requiredFields.filter(
        (field) => !persona[field as keyof LearningPersona],
      );
