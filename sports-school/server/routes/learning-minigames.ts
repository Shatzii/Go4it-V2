...
router.get('/grade/:gradeLevel', async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to fetch mini-games by grade level' });
  }
});
...
router.get('/difficulty/:difficulty', async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to fetch mini-games by difficulty' });
  }
});
...
router.get('/user/:userId/progress', async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to fetch user mini-game progress' });
  }
});
...
router.post('/user/:userId/progress/:gameId', async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.completed === true) {
      ...
      const updatedProgress = await (storage as any).completeMiniGame(
        userId,
        gameId,
        score,
        timeSpent,
      ...
      if (existingProgress) {
        ...
        const updatedProgress = await (storage as any).updateUserMiniGameProgress(
          existingProgress.id,
          validatedData,
        );
        return res.json({ success: true, data: updatedProgress });
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to update user mini-game progress' });
  }
});
...
router.post('/user/:userId/activity/:gameId', async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to record user mini-game activity' });
  }
});
...
router.get('/user/:userId/activity/:gameId', async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to fetch user mini-game activities' });
  }
});
...
router.get('/user/:userId/recommendations', async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to fetch mini-game recommendations' });
  }
});
...
router.get('/top-performing', async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to fetch top performing mini-games' });
  }
});
