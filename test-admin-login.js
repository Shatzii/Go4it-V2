...
try {
  const loginResult = execSync(`curl -s -X POST http://localhost:5000/api/auth/login \\
  ...
  if (loginData.token) {
    ...
    const meResult = execSync(`curl -s -H "Authorization: Bearer ${loginData.token}" http://localhost:5000/api/auth/me`, 
      { encoding: 'utf8' });
    ...
    if (meData.role === 'admin') {
      ...
      const adminResult = execSync(`curl -s http://localhost:5000/admin`, 
        { encoding: 'utf8' });
