...
class ContextualHelpSystem {
  ...
  autoDetectHelpPoints() {
    ...
    // Navigation elements
    const navElements = document.querySelectorAll('nav:not([data-help-context]), .sidebar:not([data-help-context])');
    ...
    const companionElements = document.querySelectorAll('#companion-container:not([data-help-context])');
          ...
          for (const node of mutation.addedNodes) {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (node.tagName === 'DIV' || node.tagName === 'SECTION' || 
                  node.tagName === 'FORM' || node.tagName === 'NAV') {
                shouldRescan = true;
                break;
              }
            }
  ...
  showHelpBubble(context, anchorElement = null) {
    ...
    helpBubble.querySelector('.help-bubble-message').textContent = character.greetings[greetingIndex];
