...
export class AdvancedScraper {
    ...
    for (let attempt = 0; attempt <= this.config.retryConfig.maxRetries; attempt++) {
      try {
        ...
        if (response.status === 200) {
          ...
          throw new Error(`Access forbidden (403) - Site may require authentication or have anti-bot measures`);
        ...
        console.log(`Request attempt ${attempt + 1} failed, retrying in ${this.config.retryConfig.retryDelay}ms...`);
  ...
  async scrapeESPN(sport: string = 'basketball', classYear: string = '2025'): Promise<ScrapingResult<any>> {
    try {
      ...
      for (const url of espnUrls) {
        try {
          ...
          const athleteElements = $('div, article, section').filter((i, el) => {
            ...
            return text.includes('recruit') || text.includes('player') || text.includes('athlete') || 
                   text.includes('commit') || text.includes('basketball') || text.includes('football');
          });
  ...
  async scrapeEuropeanSports(sport: string = 'basketball', country?: string): Promise<ScrapingResult<any>> {
    try {
