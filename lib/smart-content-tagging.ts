...
export class SmartContentTagger {
  ...
  private async analyzeVideo(
    ...
    const aiResponse = await this.aiManager.generateResponse(prompt, {
      filePath,
      fileType: 'video',
    });
  ...
  private parseAIResponse(response: string, analysis: ContentAnalysis): void {
    ...
    for (const line of lines) {
      ...
      if (lowerLine.includes('setting:')) {
        ...
        if (
          setting &&
          ['practice', 'game', 'training', 'competition', 'scrimmage'].includes(setting)
        ) {
  ...
  private generateAutoCategories(analysis: ContentAnalysis): void {
    ...
    if (analysis.context.setting !== 'unknown') {
      categories.push(
        analysis.context.setting.charAt(0).toUpperCase() + analysis.context.setting.slice(1),
      );
    }
  ...
  async bulkAnalyze(
    files: { path: string; name: string; type: 'video' | 'image' | 'document' }[],
  ): Promise<ContentAnalysis[]> {
