...
export class AIEnhancer {
  ...
  private modelPerformance: Map<string, { accuracy: number; responseTime: number; uses: number }> = new Map()
  ...
  private initializeSportSpecificModels() {
    const sports = [
      ...
      'track', 'swimming', 'volleyball', 'gymnastics', 'wrestling', 'hockey'
    ]
  ...
  async optimizeModels(): Promise<{ optimized: string[]; improvements: Record<string, number> }> {
    ...
    for (const [sport, model] of this.sportSpecificModels.entries()) {
      ...
      if (performance && learningData && learningData.length > 10) {
        ...
        const avgAccuracy = learningData.reduce((sum, entry) => sum + entry.accuracy, 0) / learningData.length
        const recentAccuracy = learningData.slice(-10).reduce((sum, entry) => sum + entry.accuracy, 0) / 10
