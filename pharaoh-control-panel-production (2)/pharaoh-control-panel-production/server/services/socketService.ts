...
export class SocketService {
  ...
  private async collectAndBroadcastMetrics(): Promise<void> {
    ...
    for (const serverId of serverIds) {
      ...
      try {
        await storage.createServerMetric({
          userId: 'demo-user',
          serverId: serverId,
          name: 'cpu_usage',
          value: metrics.cpu.toString(),
          change: (Math.random() * 10 - 5).toFixed(2),
          status: metrics.cpu > 80 ? 'critical' : metrics.cpu > 60 ? 'attention' : 'healthy'
        });
      } catch (error: any) {
        // Silently handle database errors during demo mode
      }
    }
  }

  private async collectAndBroadcastLogs(): Promise<void> {
