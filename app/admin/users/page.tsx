  ...
  const handleVerifyUser = async (userId: string, verify: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUsers(
          users.map((user) => (user.id === userId ? { ...user, isVerified: verify } : user)),
        );
          ...
          {filteredUsers.map((user) => (
            ...
            >
              ...
              <div className="flex items-center mb-4">
                ...
                <div className="flex-1 ml-4">
                  ...
                  <p className="text-slate-400 text-sm">
                    {user.position || 'Athlete'} • {user.sport || 'Multi-Sport'}
                  </p>
              ...
              <div className="mb-4 text-center">
                <div
                  className={`text-3xl font-bold ${user.garScore && user.garScore >= 8 ? 'neon-text neon-glow' : user.garScore ? 'text-blue-400' : 'text-slate-500'}`}
                >
              ...
              {user.garScore && (
                <div className="space-y-2 mb-4">
                  ...
                  {['Speed', 'Agility', 'Strength', 'Technique'].map((metric, index) => {
                    const value = user.garScore
                      ? Math.min(10, user.garScore + (Math.random() * 2 - 1))
                      : 0;
                    return (
                      <div key={metric} className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-sm font-medium ${value >= 8 ? 'neon-text' : value >= 6 ? 'text-blue-400' : value >= 4 ? 'text-yellow-400' : 'text-red-400'}`}
                          >
                            {value.toFixed(1)}
                          </span>
                        </div>
                  ...
                  <tr key={user.id} className="border-b border-slate-700/50 hover:bg-slate-800/30">
                    ...
                    <td className="p-4">
                      {user.garScore ? (
                        <div className="flex items-center gap-2">
                          <span
                            className={`font-bold ${user.garScore >= 8 ? 'neon-text' : user.garScore >= 6 ? 'text-blue-400' : user.garScore >= 4 ? 'text-yellow-400' : 'text-red-400'}`}
                          >
