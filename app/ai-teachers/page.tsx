...
export default function AITeachers() {
  ...
  const aiTeachers = [
    ...
    },
    {
      ...
      status: 'online',
      studentsHelped: 856,
      description: 'Brings literature to life with engaging storytelling and creative writing techniques',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-400">Get personalized tutoring support from our AI teaching assistants</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {aiTeachers.map((teacher) => (
            <div key={teacher.id} className="bg-slate-800 rounded-lg overflow-hidden hover:bg-slate-750 transition-colors">
              {/* Teacher Header */}
              <div className="p-6 border-b border-slate-700">
                <div className="flex items-center space-x-4">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${getColorClasses(teacher.color)}`}>
                    <teacher.icon className="w-6 h-6" />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center space-x-2">
                      <h3 className="text-lg font-semibold text-white">{teacher.name}</h3>
                      <div className={`w-3 h-3 rounded-full ${getStatusColor(teacher.status)}`}></div>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-4">
                    <div className="text-sm text-slate-400">
                      <span className="font-medium">{teacher.studentsHelped.toLocaleString()}</span> students helped
                    </div>
                    <span className={`px-2 py-1 rounded-full text-xs ${
                      teacher.status === 'online' ? 'bg-green-900 text-green-300' : 'bg-gray-900 text-gray-300'
                    }`}>
                      {teacher.status}
                    </span>
                  </div>
