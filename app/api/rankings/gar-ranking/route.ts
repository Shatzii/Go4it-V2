  ...
  try {
    ...
    if (sport && sport !== 'all') {
      filteredAthletes = filteredAthletes.filter((athlete) => {
        ...
        if (filterSport === 'football' || filterSport === 'american football') {
          ...
          return (
            athleteSport?.includes('soccer') ||
            (athleteSport?.includes('football') && !athleteSport?.includes('american'))
          );
        } else {
    ...
    if (region && region !== 'all') {
      filteredAthletes = filteredAthletes.filter((athlete) => {
        ...
        if (regionFilter === 'usa') {
          ...
          return (
            athleteCountry?.includes(regionFilter) ||
            athlete.region?.toLowerCase().includes(regionFilter)
          );
        }
      });
    }
    ...
    if (gender && (sport === 'basketball' || sport === 'soccer')) {
      filteredAthletes = filteredAthletes.filter((athlete) => {
        ...
        const athleteGender =
          athlete.gender?.toLowerCase() || athlete.position?.toLowerCase().includes('women')
            ? 'women'
            : 'men';
...
function generateRankingAnalytics(athletes: AthleteData[]) {
  ...
  const avgGAR =
    athletes.reduce((sum, athlete) => sum + (athlete.garScore || 0), 0) / athletes.length;
