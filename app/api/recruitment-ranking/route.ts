...
const SAMPLE_RANKINGS: AthleteRanking[] = [
  ...
  },
  {
    ...
    highlights: ['Regional Player of the Year', 'Olympic Development Program', 'Club National Championship'],
    stats: {
      goals: 31,
      assists: 15,
      minutesPlayed: 1680,
  ...
  },
  {
    ...
    highlights: ['All-State Selection', 'Club National Championships', 'USA Volleyball High Performance'],
    stats: {
      kills: 385,
      attacks: 892,
      percentage: 0.431,
  ...
  },
  {
    ...
    highlights: ['USA Swimming National Championships', 'Olympic Trials Qualifier', 'State Record Holder'],
    stats: {
      time50Free: 22.45,
      time100Free: 49.12,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (city) {
      filteredAthletes = filteredAthletes.filter(athlete => 
        athlete.city.toLowerCase() === city.toLowerCase()
      );
    }

    // Sort by appropriate ranking
    const sortField = rankingType === 'national' ? 'nationalRank' :
                     rankingType === 'regional' ? 'regionalRank' :
                     rankingType === 'state' ? 'stateRank' :
                     rankingType === 'city' ? 'cityRank' : 'nationalRank';
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        averageGARScore: SAMPLE_RANKINGS.reduce((sum, a) => sum + a.garScore, 0) / SAMPLE_RANKINGS.length,
    ...
    return NextResponse.json(
      { error: 'Failed to fetch recruitment rankings' },
      { status: 500 }
    );
  }
}
