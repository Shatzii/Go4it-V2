...
const colleges = [
  {
    ...
    acceptance_rate: 14,
    tuition: 43022,
    sports_offered: ['Basketball', 'Soccer', 'Baseball', 'Track & Field', 'Swimming', 'Tennis', 'Golf', 'Volleyball'],
    academic_ranking: 15,
    athletic_ranking: 8,
    campus_type: 'Urban',
    mascot: 'Bruins',
  ...
  },
  {
    ...
    acceptance_rate: 8,
    tuition: 60435,
    sports_offered: ['Basketball', 'Soccer', 'Baseball', 'Track & Field', 'Swimming', 'Tennis', 'Golf', 'Lacrosse'],
    academic_ranking: 7,
    athletic_ranking: 5,
    campus_type: 'Suburban',
    mascot: 'Blue Devils',
  ...
  },
  {
    ...
    acceptance_rate: 38,
    tuition: 41070,
    sports_offered: ['Basketball', 'Soccer', 'Baseball', 'Track & Field', 'Swimming', 'Tennis', 'Golf', 'Football'],
    academic_ranking: 42,
    athletic_ranking: 3,
    campus_type: 'Urban',
    mascot: 'Longhorns',
  ...
  },
  {
    ...
    recent_achievements: ['College Football National Championship 2023', 'NCAA Swimming Championships 2022']
  },
  {
    id: 'arizona',
  ...
  try {
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        statistics: {
          averageAcceptanceRate: Math.round(colleges.reduce((sum, c) => sum + c.acceptance_rate, 0) / colleges.length),
          averageTuition: Math.round(colleges.reduce((sum, c) => sum + c.tuition, 0) / colleges.length),
          averageEnrollment: Math.round(colleges.reduce((sum, c) => sum + c.enrollment, 0) / colleges.length),
  ...
  try {
    ...
    if (filters?.academicRanking) {
      filteredColleges = filteredColleges.filter(c => 
        c.academic_ranking <= filters.academicRanking
      );
    }
    
    if (filters?.athleticRanking) {
      filteredColleges = filteredColleges.filter(c => 
        c.athletic_ranking <= filters.athleticRanking
      );
    }
    ...
    if (filters?.enrollmentRange) {
      ...
      filteredColleges = filteredColleges.filter(c => 
        c.enrollment >= min && c.enrollment <= max
      );
    }
    
    if (filters?.conferences && filters.conferences.length > 0) {
      filteredColleges = filteredColleges.filter(c => 
        filters.conferences.includes(c.conference)
      );
    }
