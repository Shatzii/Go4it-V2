...
export async function POST(request: NextRequest) {
  try {
    ...
    const highlightMoments = highlights.length > 0 ? highlights : await generateHighlightMoments(videoId, duration);

    // Create highlight reel record
    const newHighlightReel = await db.insert(highlightReels).values({
      videoId,
...
async function processHighlightReel(highlightReelId: number) {
  ...
  setTimeout(async () => {
    try {
      await db.update(highlightReels)
        .set({ 
          status: 'completed',
