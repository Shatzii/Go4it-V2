...
async function testESPNScraping() {
  try {
    ...
    const headlines = $('h2, h3').map((i, el) => $(el).text().trim()).get().slice(0, 5);
...
async function test247SportsScraping() {
  try {
    ...
    // Extract basic page information
    const title = $('title').text();
    const headlines = $('h2, h3, .headline').map((i, el) => $(el).text().trim()).get().slice(0, 5);
...
async function testCustomURL(url: string) {
  try {
    ...
    const title = $('title').text();
    const headings = $('h1, h2, h3').map((i, el) => $(el).text().trim()).get().slice(0, 10);
  ...
  try {
    ...
    for (const site of sites) {
      try {
        ...
        const recruitingContent = $('*:contains("recruit"), *:contains("ranking"), *:contains("player")').length;
        const links = $('a[href*="recruit"], a[href*="player"], a[href*="ranking"]').map((i, el) => $(el).attr('href')).get().slice(0, 5);
