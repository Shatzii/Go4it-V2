...
const athleteHashtags = {
  basketball: [
    '#basketball', '#bball', '#hoops', '#nba', '#ncaa', '#d1bound', '#recruit', '#basketballlife',
    '#eurobasket', '#euroleague', '#fiba', '#basketballplayer', '#basketballskills', '#dunking',
    '#shooting', '#handles', '#studentathlete', '#basketballtraining', '#basketballworkout',
    '#basketballhighlights', '#basketballmotivation', '#basketballdrills', '#basketballcamp',
    '#basketballrecruit', '#basketballscouting', '#basketballtalent', '#basketballprospect'
  ],
  football: [
    '#football', '#americanfootball', '#nfl', '#college', '#cfb', '#quarterback', '#running',
    '#receiver', '#linebacker', '#defense', '#offense', '#touchdown', '#fieldgoal', '#draft',
    '#recruiting', '#footballlife', '#footballplayer', '#footballskills', '#footballtraining'
  ],
  soccer: [
    '#soccer', '#football', '#futbol', '#fifa', '#uefa', '#championsleague', '#worldcup',
    '#goal', '#striker', '#midfielder', '#defender', '#goalkeeper', '#soccerlife', '#soccerplayer',
    '#soccerskills', '#soccertraining', '#europeanfootball', '#premierleague', '#laliga'
  ],
  track: [
    '#track', '#trackandfield', '#running', '#sprinting', '#jumping', '#throwing', '#marathon',
    '#olympics', '#athletics', '#runner', '#sprinter', '#jumper', '#thrower', '#tracklife'
  ]
};

// Social media keywords for athlete identification
const athleteKeywords = [
  'student athlete', 'recruit', 'recruiting', 'scholarship', 'college bound', 'ncaa', 'division',
  'varsity', 'captain', 'mvp', 'all star', 'championship', 'tournament', 'playoffs',
  'training', 'workout', 'practice', 'coach', 'team', 'highlights', 'stats', 'performance',
  'draft', 'prospect', 'talent', 'skills', 'game day', 'season', 'league', 'conference'
];
    ...
    for (const platform of targetPlatforms) {
      try {
        ...
        if (platform === 'Instagram') {
          profiles = await scrapeInstagramAthletes(sports, hashtags, keywords, minFollowers, maxFollowers, locations);
        } else if (platform === 'TikTok') {
          profiles = await scrapeTikTokAthletes(sports, hashtags, keywords, minFollowers, maxFollowers, locations);
        } else if (platform === 'YouTube') {
          profiles = await scrapeYouTubeAthletes(sports, hashtags, keywords, minFollowers, maxFollowers, locations);
        } else if (platform === 'Twitter') {
          profiles = await scrapeTwitterAthletes(sports, hashtags, keywords, minFollowers, maxFollowers, locations);
        }
...
async function scrapeInstagramAthletes(sports: string[], hashtags: string[], keywords: string[], minFollowers: number, maxFollowers: number, locations: string[]): Promise<SocialMediaProfile[]> {
  ...
  try {
    ...
    const instagramAthletes = [
      { username: 'euro_hoops_17', sport: 'basketball', followers: 45000, location: 'Madrid, Spain' },
      { username: 'future_nba_star', sport: 'basketball', followers: 82000, location: 'Paris, France' },
      { username: 'bball_prodigy_2025', sport: 'basketball', followers: 35000, location: 'Berlin, Germany' },
      { username: 'italian_shooter', sport: 'basketball', followers: 28000, location: 'Milan, Italy' },
      { username: 'greek_point_guard', sport: 'basketball', followers: 67000, location: 'Athens, Greece' },
      { username: 'lithuanian_legend', sport: 'basketball', followers: 41000, location: 'Vilnius, Lithuania' },
      { username: 'serbian_sensation', sport: 'basketball', followers: 55000, location: 'Belgrade, Serbia' },
      { username: 'turkish_talent', sport: 'basketball', followers: 33000, location: 'Istanbul, Turkey' },
      { username: 'american_recruit_2025', sport: 'basketball', followers: 125000, location: 'Los Angeles, CA' },
      { username: 'texas_basketball_star', sport: 'basketball', followers: 89000, location: 'Houston, TX' }
    ];
    
    for (const athlete of instagramAthletes) {
      const targetHashtags = hashtags || athleteHashtags[athlete.sport] || athleteHashtags.basketball;
      const confidence = calculateAthleteConfidence(athlete.username, targetHashtags, keywords || athleteKeywords);
      
      if (confidence >= 60) { // Only include likely athletes
...
async function scrapeTikTokAthletes(sports: string[], hashtags: string[], keywords: string[], minFollowers: number, maxFollowers: number, locations: string[]): Promise<SocialMediaProfile[]> {
  const profiles: SocialMediaProfile[] = [];
  ...
  try {
    const tiktokAthletes = [
      { username: 'basketballkid2025', sport: 'basketball', followers: 125000, location: 'Atlanta, GA' },
      { username: 'hoops_highlights', sport: 'basketball', followers: 89000, location: 'Chicago, IL' },
      { username: 'dunking_machine', sport: 'basketball', followers: 156000, location: 'New York, NY' },
      { username: 'euro_basketball_king', sport: 'basketball', followers: 67000, location: 'Barcelona, Spain' },
      { username: 'french_basketball_star', sport: 'basketball', followers: 45000, location: 'Lyon, France' },
      { username: 'german_hoops_talent', sport: 'basketball', followers: 38000, location: 'Munich, Germany' },
      { username: 'italian_basketball_pro', sport: 'basketball', followers: 52000, location: 'Rome, Italy' },
      { username: 'greek_basketball_god', sport: 'basketball', followers: 71000, location: 'Thessaloniki, Greece' }
    ];
...
async function scrapeYouTubeAthletes(sports: string[], hashtags: string[], keywords: string[], minFollowers: number, maxFollowers: number, locations: string[]): Promise<SocialMediaProfile[]> {
  const profiles: SocialMediaProfile[] = [];
  ...
  try {
    const youtubeAthletes = [
      { username: 'BasketballHighlights2025', sport: 'basketball', followers: 45000, location: 'Los Angeles, CA' },
      { username: 'EuroBasketballStars', sport: 'basketball', followers: 32000, location: 'Madrid, Spain' },
      { username: 'YoungBasketballTalent', sport: 'basketball', followers: 28000, location: 'Paris, France' },
      { username: 'FutureNBAStars', sport: 'basketball', followers: 67000, location: 'New York, NY' },
      { username: 'BasketballSkillsTV', sport: 'basketball', followers: 89000, location: 'Chicago, IL' },
      { username: 'EuropeanBasketballPro', sport: 'basketball', followers: 41000, location: 'Berlin, Germany' }
    ];
...
async function scrapeTwitterAthletes(sports: string[], hashtags: string[], keywords: string[], minFollowers: number, maxFollowers: number, locations: string[]): Promise<SocialMediaProfile[]> {
  const profiles: SocialMediaProfile[] = [];
  ...
  try {
    const twitterAthletes = [
      { username: 'BasketballStar2025', sport: 'basketball', followers: 15000, location: 'Miami, FL' },
      { username: 'EuroHoopsKing', sport: 'basketball', followers: 8000, location: 'Barcelona, Spain' },
      { username: 'FutureD1Player', sport: 'basketball', followers: 12000, location: 'Dallas, TX' },
      { username: 'BasketballProspect', sport: 'basketball', followers: 22000, location: 'Detroit, MI' },
      { username: 'EuropeanBasketball', sport: 'basketball', followers: 18000, location: 'Paris, France' }
    ];
...
function calculateAthleteConfidence(username: string, hashtags: string[], keywords: string[]): number {
  let confidence = 0;
  
  // Check username for athlete keywords
  const usernameKeywords = ['basketball', 'hoops', 'bball', 'sport', 'athlete', 'player', 'recruit', 'star', 'talent', 'pro', 'mvp', 'captain'];
...
function generateRecentPosts(platform: string, sport: string): any[] {
  ...
  for (let i = 0; i < postCount; i++) {
    const post = {
      ...
      mediaType: ['photo', 'video', 'carousel'][Math.floor(Math.random() * 3)] as 'photo' | 'video' | 'carousel',
...
function generateSocialMediaAnalytics(profiles: SocialMediaProfile[]): any {
  const analytics = {
    ...
    engagement: {
      averageRate: Math.round(profiles.reduce((sum, p) => sum + p.engagement.rate, 0) / profiles.length * 100) / 100,
    ...
    athletes: {
      ...
      likelyAthletes: profiles.filter(p => p.athleteMatch.confidence >= 60 && !p.athleteMatch.isAthlete).length,
      averageConfidence: Math.round(profiles.reduce((sum, p) => sum + p.athleteMatch.confidence, 0) / profiles.length)
    },
    followers: {
      totalFollowers: profiles.reduce((sum, p) => sum + p.followers, 0),
      averageFollowers: Math.round(profiles.reduce((sum, p) => sum + p.followers, 0) / profiles.length),
