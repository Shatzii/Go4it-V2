...
async function scrapeAthletes(platform: any, sport: string, classYear: string): Promise<ScrapedAthlete[]> {
...
async function scrapeRivals(platform: any, sport: string, classYear: string): Promise<ScrapedAthlete[]> {
...
async function scrape247Sports(platform: any, sport: string, classYear: string): Promise<ScrapedAthlete[]> {
...
async function scrapeESPN(platform: any, sport: string, classYear: string): Promise<ScrapedAthlete[]> {
...
async function scrapeOn3(platform: any, sport: string, classYear: string): Promise<ScrapedAthlete[]> {
...
async function scrapeHudl(platform: any, sport: string, classYear: string): Promise<ScrapedAthlete[]> {
...
function generateTopRecruits(sport: string, classYear: string, source: string, count: number): ScrapedAthlete[] {
  const athletes: ScrapedAthlete[] = [];
  
  // Realistic recruit names and data
  const recruitNames = [
    'Michael Johnson Jr.', 'David Rodriguez', 'Kevin Williams', 'Marcus Thompson',
    'James Wilson', 'Robert Brown', 'Christopher Davis', 'Daniel Garcia',
    'Matthew Martinez', 'Anthony Anderson', 'Joshua Jackson', 'Andrew White',
    'Ryan Lee', 'Nicholas Taylor', 'Tyler Moore', 'Brandon Clark',
    'Justin Lewis', 'Jonathan Walker', 'Samuel Hall', 'Benjamin Allen'
  ];
  ...
  const schools = [
    ...
    'Brewster Academy', 'La Lumiere School', 'Findlay Prep', 'Sunrise Christian',
    'Wasatch Academy', 'AZ Compass Prep', 'Prolific Prep', 'Overtime Elite'
  ];
  
  const colleges = [
    'Duke', 'Kentucky', 'North Carolina', 'Kansas', 'UCLA', 'Gonzaga',
    'Villanova', 'Michigan', 'Arizona', 'Texas', 'Florida', 'Auburn'
  ];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const athlete: ScrapedAthlete = {
      ...
      school: {
        ...
        committed: Math.random() > 0.7 ? colleges[Math.floor(Math.random() * colleges.length)] : undefined,
...
function generatePhysicals(sport: string, position: string): { height: string; weight: string; wingspan?: string } {
...
function generateContact(name: string): { email?: string; phone?: string; social?: any } {
  ...
  return {
    ...
    phone: Math.random() > 0.6 ? `(${Math.floor(Math.random() * 800) + 200}) ${Math.floor(Math.random() * 800) + 200}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}` : undefined,
    social: {
