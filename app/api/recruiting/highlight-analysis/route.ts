  ...
  try {
    return NextResponse.json({
      ...
      metadata: {
        ...
        analysisPoints: ['Technical skills', 'Physical attributes', 'Tactical awareness', 'Peak performance', 'Consistency']
      }
    });
  } catch (error) {
    return NextResponse.json({
...
export async function PUT(request: Request) {
  try {
    ...
    const analyses = highlightAnalysis.filter(h => 
      athleteIds.includes(h.athleteId)
    );
...
async function generateHighlightAnalysis(videoUrl: string, sport: string, position: string) {
  ...
  return {
    ...baseAnalysis,
    overall: Math.round((baseAnalysis.technical + baseAnalysis.physical + baseAnalysis.tactical) / 3),
...
function generateComparison(analyses: any[], comparisonType: string) {
  const comparison = {
    summary: {
      ...
      averageOverall: Math.round(analyses.reduce((sum, a) => sum + a.analysis.technical.overall, 0) / analyses.length),
