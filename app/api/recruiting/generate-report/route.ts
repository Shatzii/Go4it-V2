  ...
  try {
    ...
    if (!athlete.isVerified) {
      return NextResponse.json(
        { error: 'Athlete must be verified to generate recruiting report' },
        { status: 400 },
      );
    }
    ...
    if (analyses.length === 0) {
      return NextResponse.json(
        { error: 'No GAR analyses found for this athlete' },
        { status: 400 },
      );
    }
    ...
    const recruitingReport = await generateComprehensiveReport(
      athlete,
      analyses,
      performanceMetrics,
    );
  ...
  analyses.slice(0, 3).forEach((analysis) => {
    ...
    if (feedback && typeof feedback === 'string') {
      ...
      sentences.forEach((sentence) => {
        if (
          category === 'technical' &&
          (sentence.includes('form') || sentence.includes('technique'))
        ) {
          examples.push(sentence.trim());
        } else if (
          category === 'athleticism' &&
          (sentence.includes('speed') || sentence.includes('power'))
        ) {
          examples.push(sentence.trim());
        } else if (
          category === 'awareness' &&
          (sentence.includes('decision') || sentence.includes('positioning'))
        ) {
          examples.push(sentence.trim());
        }
      });
...
function generateRecruitingHighlights(athlete: any, metrics: any): string[] {
  ...
  if (athlete.isVerified) {
    highlights.push(
      'Officially verified athlete through Go4It Sports comprehensive analysis system',
    );
  }
