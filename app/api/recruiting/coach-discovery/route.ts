...
const coachProfiles = [
  {
    ...
    coachingProfile: {
      ...
      recruitingPriorities: ['Character', 'Basketball IQ', 'Defensive mindset', 'Academic excellence'],
      playerTypes: {
...
export async function POST(request: Request) {
  try {
    ...
    const matches = athleteDatabase.filter(athlete => {
...
function calculateSystemFit(athlete: any, systemType: string): number {
  ...
  switch (systemType) {
    case 'motion-offense':
      return Math.round((skills.basketballIQ * 0.3) + (skills.passing * 0.25) + (skills.shooting * 0.25) + (skills.versatility * 0.2));
    case 'spread-offense':
      return Math.round((skills.shooting * 0.4) + (skills.ballHandling * 0.3) + (skills.basketballIQ * 0.3));
    case 'high-low-post':
