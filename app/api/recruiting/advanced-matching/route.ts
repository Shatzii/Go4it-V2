  ...
  try {
    return NextResponse.json({
      ...
      metadata: {
        ...
        dataPoints: ['Coaching schemes', 'Roster limits', 'Position needs', 'Transfer portal', 'Graduation impact']
      }
    });
  } catch (error) {
    return NextResponse.json({
...
export async function POST(request: Request) {
  try {
    ...
    const matches = rosterAnalysis.map(school => {
      ...
      const competition = calculateCompetition(school.recruitingClass.targets, school.recruitingClass.available);
      
      // Overall match score
      const matchScore = Math.round(
        (schemeFit * 0.4) + 
        (rosterOpportunity * 0.35) + 
        (competition * 0.25)
      );
