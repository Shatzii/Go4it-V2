...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!sport || !level) {
      return NextResponse.json({ error: 'Sport and level are required' }, { status: 400 });
    }
    ...
    basketball: [
      {
        ...
        modifications_for_adhd:
          'Use bright colored cones, set 5-minute timers, celebrate small wins',
      },
      {
        name: 'Shooting Form',
