...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'AI Coach integration failed' },
      { status: 500 }
    );
  }
}
...
async function handleGarDataProcessing(user: any, data: any) {
  ...
  if (analysisData?.technicalSkills >= 80) {
    ...
    coachingInsights.voice_coaching_points.push('Your form is looking great! Let\'s talk about maintaining that consistency under pressure.');
  } else if (analysisData?.technicalSkills < 70) {
    coachingInsights.improvements.push('Technical skills need development');
    coachingInsights.voice_coaching_points.push('I notice some areas where we can refine your technique. Want to work on specific drills?');
  }

  // Analyze athleticism
  if (analysisData?.athleticism >= 80) {
    coachingInsights.strengths.push('Strong athletic performance');
    coachingInsights.voice_coaching_points.push('Your athleticism is a real strength. How can we leverage that in game situations?');
  } else if (analysisData?.athleticism < 70) {
    coachingInsights.improvements.push('Athletic conditioning needed');
    coachingInsights.voice_coaching_points.push('Let\'s discuss a conditioning program to boost your athletic performance.');
  }
...
async function handleCoachingResponse(user: any, data: any) {
  ...
  try {
    ...
    const coachingResponse = aiResponse.choices[0]?.message?.content || 'I need more information to help you with that.';
