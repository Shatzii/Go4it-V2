...
if (process.env.NEXT_PHASE === 'phase-production-build') {
  return NextResponse.json({ error: 'Service temporarily unavailable during build' }, { status: 503 });
    ...
    const existingReaction = db.prepare(`
      ...
      WHERE post_id = ? AND student_id = ? AND reaction_type = 'like'
    `).get(params.postId, studentId);
    ...
    if (existingReaction) {
      ...
      // Add reaction (like)
      db.prepare(`
        INSERT INTO collaboration_reactions (post_id, student_id, reaction_type)
        VALUES (?, ?, 'like')
      `).run(params.postId, studentId);
