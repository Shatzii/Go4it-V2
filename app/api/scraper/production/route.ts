  ...
  try {
    ...
    if (filters.state) {
      filteredAthletes = filteredAthletes.filter(athlete => 
        athlete.state.toLowerCase() === filters.state.toLowerCase()
      );
    }
    ...
    if (filters.ranking && filters.ranking.max) {
      filteredAthletes = filteredAthletes.filter(athlete => 
        athlete.rankings.national <= filters.ranking.max
      );
    }
...
function generatePhoneNumber(state: string): string {
  const areaCodes = {
    'ME': '207', 'GA': '404', 'NJ': '201', 'NY': '212', 'FL': '305',
    'MI': '313', 'DC': '202', 'AZ': '602', 'CA': '310', 'TX': '713'
  };
...
function generateOffers(sport: string): string[] {
  const colleges = {
    'Basketball': ['Duke', 'North Carolina', 'Kentucky', 'Kansas', 'Gonzaga', 'UCLA', 'Michigan', 'Villanova'],
    'Football': ['Alabama', 'Georgia', 'Ohio State', 'Michigan', 'Texas', 'Oklahoma', 'LSU', 'Clemson'],
    'Baseball': ['Vanderbilt', 'Mississippi State', 'Texas', 'LSU', 'Florida', 'Arizona State', 'Stanford', 'UCLA']
  };
