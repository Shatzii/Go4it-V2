...
export async function GET(request: NextRequest) {
  try {
    ...
    if (platform) {
      const platformData = socialMetrics.platforms.find(p => 
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Social media action failed' },
      { status: 500 }
    );
  }
}
...
async function handleContentGeneration(userId: string, content: any) {
  ...
  const selectedType = content.type || contentTypes[Math.floor(Math.random() * contentTypes.length)];
