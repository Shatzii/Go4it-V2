      ...
      <Tabs defaultValue="featured" className="w-full" onValueChange={setActiveTab}>
        ...
        <TabsContent value="featured" className="mt-0">
          <div className="relative">
            ...
            {/* Navigation Dots */}
            {featuredBlogPosts.filter(post => post.featured).length > 0 && (
              <div className="flex justify-center mt-4 space-x-2">
                {Array.from({ length: Math.min(Math.ceil(featuredBlogPosts.filter(post => post.featured).length / 4), 3) }).map((_, i) => (
                  <div 
                    key={i} 
