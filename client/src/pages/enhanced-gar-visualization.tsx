...
export function EnhancedGarVisualizationPage() {
  const { toast } = useToast();
  const [, params] = useRoute<{ id?: string }>('/enhanced-gar/:id');
  ...
  useEffect(() => {
    if (!userId && userData?.id) {
      setLocation(`/enhanced-gar/${userData.id}`);
    }
  }, [userId, userData?.id]);
...
export default EnhancedGarVisualizationPage;