...
export class RealTimeVideoAnalyzer {
  ...
  private async performLightweightAnalysis(frameInput: any): Promise<Partial<AdvancedVideoAnalysis>> {
  ...
  private updateMetrics(processingTime: number): void {
    ...
    this.metrics.buffer_health = Math.max(0, 100 - (this.frameBuffer.length / this.config.buffer_size) * 100);
  ...
  getCurrentAnalysis(): LiveAnalysisFrame | null {
    return this.analysisResults.length > 0 ? this.analysisResults[this.analysisResults.length - 1] : null;
  }
