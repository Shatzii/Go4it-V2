...
import { users, schools, courses, assignments, submissions, progress, aiSessions, payments, customers, subscriptions } from '@/shared/schema'
import type { User, NewUser, School, NewSchool, Course, NewCourse, Assignment, NewAssignment, Submission, NewSubmission, Progress, NewProgress, AiSession, NewAiSession, Payment, NewPayment, Customer, NewCustomer, Subscription, NewSubscription } from '@/shared/schema'
...
export class DrizzleStorage implements IStorage {
  ...
  async getAiSessionsByUser(userId: string): Promise<AiSession[]> {
    return await db.select().from(aiSessions)
  ...
  async updateAiSession(id: string, updates: Partial<NewAiSession>): Promise<AiSession | null> {
    const result = await db.update(aiSessions)
      .set({ ...updates, updatedAt: new Date() })
  ...
  async updateSubscription(id: string, updates: Partial<NewSubscription>): Promise<Subscription | null> {
