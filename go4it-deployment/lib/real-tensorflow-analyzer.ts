...
export class RealTensorFlowAnalyzer {
  ...
  private calculateJointAngles(poses: Pose[]): any {
    ...
    poses.forEach((pose) => {
      if (pose.keypoints.length >= 17) {
        // Standard pose keypoints
        // Calculate knee angles
        angles.leftKnee.push(
          this.calculateAngle(
    ...
    for (let i = 1; i < poses.length; i++) {
      ...
      if (currentPose.keypoints.length > 0 && previousPose.keypoints.length > 0) {
        ...
        const distance = Math.sqrt(
          Math.pow(currentCOM.x - previousCOM.x, 2) + Math.pow(currentCOM.y - previousCOM.y, 2),
        );
  ...
  private evaluateJointAngles(jointAngles: any): number {
    ...
    if (jointAngles.leftKnee && jointAngles.leftKnee.length > 0) {
      const avgAngle =
        jointAngles.leftKnee.reduce((a: number, b: number) => a + b, 0) /
        jointAngles.leftKnee.length;
    ...
    if (jointAngles.rightKnee && jointAngles.rightKnee.length > 0) {
      const avgAngle =
        jointAngles.rightKnee.reduce((a: number, b: number) => a + b, 0) /
        jointAngles.rightKnee.length;
      score += this.scoreKneeAngle(avgAngle);
      count++;
    }
